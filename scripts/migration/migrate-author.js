...
/**
 * MIGRATION SCRIPT: author
 * 
 * Consolidates: author, aboutAuthor, authorTimeline, authorAchievements, teamMember
 * Target: Author & Team
 * Category: people
 * 
 * This script migrates data from multiple source schemas into the consolidated author schema.
 */
...
async function migrateAuthor() {
  ...
  try {
    ...
    const sourceTypes = ["author","aboutAuthor","authorTimeline","authorAchievements","teamMember"];
    ...
    if (sourceDocuments.length === 0) {
      console.log('  ℹ️  No documents to migrate for author');
      return { success: true, migrated: 0 };
    }
    
    // Step 2: Transform documents to new schema structure
    const transformedDocuments = sourceDocuments.map(doc => transformDocumentAuthor(doc));
    ...
    for (const doc of transformedDocuments) {
      try {
        const result = await sanityClient.create({
          ...doc,
          _type: 'author',
          _id: doc._id ? `author-${doc._id}` : undefined,
    ...
    console.log(`✅ Successfully migrated ${results.length}/${transformedDocuments.length} documents to author`);
    return { success: true, migrated: results.length, total: transformedDocuments.length };
    
  } catch (error) {
    console.error(`❌ Migration failed for author: ${error.message}`);
    return { success: false, error: error.message };
  }
}

function transformDocumentAuthor(doc) {
  // TODO: Implement specific transformation logic for author
  ...
  // Example transformations based on author:
  
  // Add specific transformation logic for author
...
module.exports = {
  migrate: migrateAuthor,
  transform: transformDocumentAuthor
};
